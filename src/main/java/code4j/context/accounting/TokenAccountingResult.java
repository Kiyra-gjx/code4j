...
public record TokenAccountingResult(long inputTokens, long outputTokens, long totalTokens,
    ...
    public static TokenAccountingResult providerUsage(long inputTokens, long outputTokens, long totalTokens,
        ...
        return new TokenAccountingResult(inputTokens, outputTokens, totalTokens, totalTokens, 0,
                true, TokenAccountingSource.PROVIDER_USAGE, Optional.of(usageBoundary), false, Optional.empty());
    }

    public static TokenAccountingResult providerUsageWithEstimate(long inputTokens, long outputTokens,
                                                                   long totalTokens, long providerUsageTokens,
                                                                   long estimatedTokens,
                                                                   UsageBoundary usageBoundary) {
        return new TokenAccountingResult(inputTokens, outputTokens, totalTokens, providerUsageTokens,
                estimatedTokens, false, TokenAccountingSource.PROVIDER_USAGE_WITH_ESTIMATE,
                Optional.of(usageBoundary), false, Optional.empty());
    }

    public static TokenAccountingResult estimateOnly(long estimatedTokens, Optional<String> staleReason) {
        return new TokenAccountingResult(estimatedTokens, 0, estimatedTokens, 0, estimatedTokens,
                false, TokenAccountingSource.ESTIMATE_ONLY, Optional.empty(),
                staleReason.isPresent(), staleReason.or(() -> Optional.of("no provider usage available")));
    }
}
