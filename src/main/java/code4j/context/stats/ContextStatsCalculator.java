package code4j.context.stats;

import code4j.context.accounting.TokenAccountingResult;
...
public final class ContextStatsCalculator {
    ...
    public ContextStats calculate(List<ChatMessage> messages) {
        ...
        return new ContextStats(accounting, window.contextWindow(), window.outputReserve(),
                window.effectiveInput(), utilization, warningLevel(utilization));
    }

    private static ContextWarningLevel warningLevel(double utilization) {
        if (utilization >= 0.95d) return ContextWarningLevel.BLOCKED;
        if (utilization >= 0.85d) return ContextWarningLevel.CRITICAL;
        if (utilization >= 0.50d) return ContextWarningLevel.WARNING;
