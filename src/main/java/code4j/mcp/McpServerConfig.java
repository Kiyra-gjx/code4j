 ...
 * <p>
 * Each server is defined by a command to launch it, plus optional args,
 * working directory, environment variables, and timeout settings.
 * The {@code enabled} flag allows servers to be defined but turned off.
 */
public final class McpServerConfig {
    ...
    public Duration initializeTimeout() { return initializeTimeout; }

    public Duration callTimeout() { return callTimeout; }

    public Duration requestTimeout() { return callTimeout; }

    public String endpointSummary() {
        return (command + " " + String.join(" ", args)).trim();
    }

    public String commandSummary() { return endpointSummary(); }
}
