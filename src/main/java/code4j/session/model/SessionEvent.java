...
public record SessionEvent(SessionEventType type, String uuid, Instant timestamp, String sessionId, String cwd,
    ...
    public static SessionEvent message(String uuid, Instant timestamp, String sessionId, String cwd,
                                       Optional<String> parentUuid, Optional<String> logicalParentUuid,
                                       ChatMessage message) {
        return new SessionEvent(SessionEventType.MESSAGE, uuid, timestamp, sessionId, cwd,
                parentUuid, logicalParentUuid, Optional.of(message), Optional.empty(), Optional.empty());
    }
    ...
    public static SessionEvent meta(String uuid, Instant timestamp, String sessionId, String cwd,
        ...
        return new SessionEvent(draft.eventType(), uuid, timestamp, sessionId, cwd,
                parentUuid, logicalParentUuid, Optional.empty(), Optional.of(draft), Optional.empty());
    }

    public static SessionEvent compactBoundary(String uuid, Instant timestamp, String sessionId, String cwd,
                                               Optional<String> parentUuid, Optional<String> logicalParentUuid) {
        return new SessionEvent(SessionEventType.COMPACT_BOUNDARY, uuid, timestamp, sessionId, cwd,
                parentUuid, logicalParentUuid, Optional.empty(), Optional.empty(), Optional.empty());
    }
    ...
    public static SessionEvent compactBoundary(String uuid, Instant timestamp, String sessionId, String cwd,
        ...
        return new SessionEvent(SessionEventType.COMPACT_BOUNDARY, uuid, timestamp, sessionId, cwd,
                parentUuid, logicalParentUuid, Optional.empty(), Optional.empty(), Optional.of(metadata));
    }
    ...
    private static void validate(SessionEventType type, Optional<ChatMessage> message,
        ...
        if (type == SessionEventType.MESSAGE) {
            if (message.isEmpty() || meta.isPresent() || compactMetadata.isPresent()) {
                throw new IllegalArgumentException("MESSAGE event requires message and no meta/compact payload");
            }
            return;
        }
        ...
        if (message.isPresent() || compactMetadata.isPresent() || meta.isEmpty()
                || meta.orElseThrow().eventType() != type) {
            throw new IllegalArgumentException(type + " requires matching meta draft and no message/compact");
        }
    }
