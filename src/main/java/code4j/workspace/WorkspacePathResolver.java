...
/**
 * Resolves tool-supplied paths against the workspace root, normalizes them,
 * and determines whether they fall inside or outside the workspace boundary.
 * <p>
 * All builtin tools that touch the filesystem go through this resolver —
 * it's the single choke point for path safety.
 */
public final class WorkspacePathResolver {
    ...
    private static Optional<Path> parentRealPathForMissingTarget(WorkspacePathRequest request,
                                                                  Path normalizedPath, boolean exists) {
        ...
        try {
            ...
            throw new WorkspacePathException("Unable to resolve parent real path: " + parent, exception);
        }
    }
