...
/**
 * The result of validating tool input from the model.
 * <p>
 * If valid, {@code normalizedInput} contains the cleaned-up JSON (defaults filled,
 * types coerced) ready for execution. If invalid, {@code errors} explains why.
 */
public record ValidationResult(boolean valid, Optional<JsonNode> normalizedInput, List<String> errors) {
    ...
    public static ValidationResult valid(JsonNode normalizedInput) {
        return new ValidationResult(true, Optional.of(Objects.requireNonNull(normalizedInput, "normalizedInput")),
                List.of());
    }
