...
/**
 * Field-level validation helpers. Each method extracts a field from the raw JSON input,
 * validates it, and (if valid) writes the normalized value into the builder.
 */
public final class JsonFieldValidators {
    ...
    private static void integer(JsonNode input, ValidatedInputBuilder builder, String field,
    ...
    static void optionalStringArray(JsonNode input, ValidatedInputBuilder builder,
                                    String field, boolean defaultEmpty) {
        JsonNode node = field(input, field);
        if (node == null || node.isNull()) {
            if (defaultEmpty) {
    ...
    static void enumString(JsonNode input, ValidatedInputBuilder builder, String field,
                           Set<String> allowedValues, boolean required) {
